...
view_only			= int(ph.getParam( "viewOnly",   	  		False ))>0 		
...
# exit()
if (not outputOnly): 
    tiCr.addData(inputx, y, all_ppos)
...
ppos = tf.placeholder(tf.float32, shape=[None, None])
...
def grid_sampler(img, pXx, pXy, interp='bilinear'):
    """
    ...
    W = tf.shape(img)[0]
    H = tf.shape(img)[1]
    max_i = tf.cast(W, 'int32')
    max_j = tf.cast(H, 'int32')
    zero = tf.zeros([], dtype='int32')
    # rescale x and y to [0, W/H]
    pXx = tf.cast(pXx, 'float32')
    pXy = tf.cast(pXy, 'float32')
    pXx = pXx * tf.cast(max_i, 'float32')
    pXy = pXy * tf.cast(max_j, 'float32')
    if interp == 'bilinear':
        # compute neighbors, save as int32
        cil = tf.cast(tf.floor(pXx), 'int32')
        cir = cil + 1
        cjb = tf.cast(tf.floor(pXy), 'int32')
        cjt = cjb + 1

        # clamp: [0, max-1]
        cil = tf.clip_by_value(cil, zero, max_i - 1)
        cir = tf.clip_by_value(cir, zero, max_i - 1)
        cjb = tf.clip_by_value(cjb, zero, max_j - 1)
        cjt = tf.clip_by_value(cjt, zero, max_j - 1)

        bl_id = tf.math.add(tf.math.scalar_mul(W, cjb), cil)    # (np, 1)
        br_id = tf.math.add(tf.math.scalar_mul(W, cjb), cir)
        tl_id = tf.math.add(tf.math.scalar_mul(W, cjt), cil)
        tr_id = tf.math.add(tf.math.scalar_mul(W, cjt), cir)
        
        Ibl = tf.reshape(get_pixel_value(img, bl_id), shape=[-1, 2])                       # (np, 1, 2)
        Ibr = tf.reshape(get_pixel_value(img, br_id), shape=[-1, 2])
        Itl = tf.reshape(get_pixel_value(img, tl_id), shape=[-1, 2])
        Itr = tf.reshape(get_pixel_value(img, tr_id), shape=[-1, 2])
        
        cxl = tf.cast(cil, 'float32')                           # (np, 1)
        cxr = tf.cast(cir, 'float32')
        cyb = tf.cast(cjb, 'float32')
        cyt = tf.cast(cjt, 'float32')
        
        wa = (cxr-pXx) * (cyt-pXy)                              # (np, 1)
        wb = (cxr-pXx) * (pXy-cyb)
        wc = (pXx-cxl) * (cyt-pXy)
        wd = (pXx-cxl) * (pXy-cyb)
        ...
        # i, j: index
        # x, y: location
        dx = tf.cast(1., 'float32') / tf.cast(W, 'float32')
        inv_dx = tf.cast(W, 'float32')
        cic, cjc= tf.cast(tf.floor(pXx), 'int32'), tf.cast(tf.floor(pXy), 'int32')
        cxc, cyc = tf.cast(cic, 'float32'), tf.cast(cjc, 'float')

        cil, cir = cic - 1, cic + 1
        cjb, cjt = cjc - 1, cjc + 1

        # ccx_pos, ccy
        X_eval_x, X_eval_y = pXx - (cxc - dx), pXy - (cyc - dx)
        wxr = tf.abs(.5*(X_eval_x-2.)*(X_eval_x-2.)+1.5*(X_eval_x-2.)+1.125)
        wxc = tf.abs(-(X_eval_x-1.)*(X_eval_x-1.)+.75)
        wxl = tf.abs(.5*X_eval_x*X_eval_x -1.5*X_eval_x+1.125)

        wyt = tf.abs(.5*(X_eval_y-2.)*(X_eval_y-2.)+1.5*(X_eval_y-2.)+1.125)
        wyc = tf.abs(-(X_eval_y-1.)*(X_eval_y-1.)+.75)
        wyb = tf.abs(.5*X_eval_y*X_eval_y -1.5*X_eval_y+1.125)
        
        wtl, wtc, wtr = wyt * wxl, wyt * wxc, wyt * wxr
        wcl, wcc, wcr = wyc * wxl, wyc * wxc, wyc * wxr
        wbl, wbc, wbr = wyb * wxl, wyb * wxc, wyb * wxr

        ctl_id, ctc_id, ctr_id = W * cjt + cil, W * cjt + cic, W * cjt + cir
        ccl_id, ccc_id, ccr_id = W * cjc + cil, W * cjc + cic, W * cjc + cir
        cbl_id, cbc_id, cbr_id = W * cjb + cil, W * cjb + cic, W * cjb + cir
        
        Itl, Itc, Itr = tf.reshape(get_pixel_value(img, ctl_id), shape=[-1, 2]), tf.reshape(get_pixel_value(img, ctc_id), shape=[-1, 2]), tf.reshape(get_pixel_value(img, ctr_id), shape=[-1, 2])
        Icl, Icc, Icr = tf.reshape(get_pixel_value(img, ccl_id), shape=[-1, 2]), tf.reshape(get_pixel_value(img, ccc_id), shape=[-1, 2]), tf.reshape(get_pixel_value(img, ccr_id), shape=[-1, 2])
        Ibl, Ibc, Ibr = tf.reshape(get_pixel_value(img, cbl_id), shape=[-1, 2]), tf.reshape(get_pixel_value(img, cbc_id), shape=[-1, 2]), tf.reshape(get_pixel_value(img, cbr_id), shape=[-1, 2])
        out = tf.add_n([    wtl*Itl, wtc*Itc, wtr*Itr, wcl*Icl, wcc*Icc, wcr*Icr, wbl*Ibl, wbc*Ibc, wbr*Ibr])
...
def dv_mul(grid_v):
    dt = 1.
    shape = tf.shape(grid_v)
    grid_v = tf.slice(grid_v,[0,0],[shape[0],int(n_outputChannels*n_input/n_inputChannels)]) # use only the velocity
    pposx, pposy = tf.slice(ppos, [0, 0], [-1, 1]), tf.slice(ppos, [0, 1],[-1, 1])
    pvels = grid_sampler(grid_v, pposx, pposy, interp='other')
    new_ppos = dt * pvels + ppos
    return pvels, new_ppos
...
if not outputOnly:
    ...
    # gen_part: [high_res, high_res, 2]
    # 		 y: [high_res, high_res, 2]
    # 		 x: [low_res,  low_res,  3]
    gt_pvels, gt_ppos = dv_mul(x)
    gen_pvels, gen_ppos = dv_mul(gen_part)
    gen_ot_loss = tf.reduce_mean(tf.abs(gt_ppos - gen_ppos))
    # pvel_gen = grid_sampler()
    # pvel_ref = grid_sampler()
    # ppos_gen = forward_particles()
    # ppos_ref = forward_particles()
    # gen_emd_loss = emd_loss()
...
def getInput(index = 1, randomtile = True, isTraining = True, batch_size = 1, useDataAugmentation = False):
    if randomtile == False:
        cur_ppos, batch_xs, batch_ys = tiCr.getFrameTiles(index) 
    else:
        # this way
        cur_ppos, batch_xs, batch_ys = tiCr.selectRandomTiles(selectionSize = batch_size, augment=useDataAugmentation)	
    batch_xs = np.reshape(batch_xs, (-1, n_input))
    batch_ys = np.reshape(batch_ys, (-1, n_output))
    return cur_ppos, batch_xs, batch_ys
...
def generateValiVel(sim_no = fromSim, frame_no = 1, outPath = test_path,imageindex = 0):
    if premadeTiles:
        ...
        if (not outputOnly):
            cur_ppos, batch_xs, _ = getInput(randomtile = False, index = (sim_no-fromSim)*frameMax + frame_no)
        else:
            batch_xs = inputx[frame_no]
        ...
        for iteration in range(trainingIters):
            ...
            if use_spatialdisc:
                for runs in range(discRuns):
                    cur_ppos, batch_xs, batch_ys = getInput(batch_size = batch_size_disc, useDataAugmentation = useDataAugmentation)
                    # print('batch_size_disc: {}'.format(batch_size_disc)) # 16
                    # print('batch_xs: {}'.format(batch_xs.shape)) # (16, 16x16x4)
                    # input('')
                    # print('batch_ys: {}'.format(batch_ys.shape,)) # (16, 4096)
                    # input('')
                    # print('cur_ppos: {}'.format(cur_ppos.shape))
                    # input('')
            ...
            for runs in range(genRuns):
                cur_ppos, batch_xs, batch_ys = getInput(batch_size = batch_size_disc, useDataAugmentation = useDataAugmentation)
                # print('cur_ppos shape: {}'.format(cur_ppos.shape))
                ...
                if use_spatialdisc:
                    ...
                    getlist = [gen_optimizer, gen_loss, disc_loss_layer, gen_l1_loss, lossTrain_gen, gen_l2_loss]
                    if False:
                        getlist.append(gt_pvels)
                        getlist.append(gen_pvels)
                    #     getlist.append(gen_ot_loss)
                    #     getlist.append(gen_ppos)
                    #     getlist.append(gt_ppos)
                    #     getlist.append(pposx_ph)
                    #     getlist.append(pposy_ph)
                    #     getlist.append(pvels_ph)
                    #     getlist.append(ppos)

                    #     getlist.append(bl_id_ph)
                    #     getlist.append(Ibl_ph)
                    #     getlist.append(cxl_ph)
                    #     getlist.append(wa_ph)
                else:
                ...
                if (useTempoD and (not useTempoL2)):
                    ...
                    if use_spatialdisc:
                        if False:
                            _, gen_cost, layer_cost, gen_l1_cost, summary, gen_l2_cost, gt_pVel, gen_pVel = result_list
                            print('gtVel: {}, genVel: {}'.format(gt_pVel.shape, gen_pVel.shape))
                            for i in range(gt_pVel.shape[0]):
                                cur_gtvel = gt_pVel[i]
                                cur_genvel = gen_pVel[i]
                                if cur_gtvel[0] == 0. or cur_gtvel[1] == 0. or cur_genvel[0] == 0. or cur_genvel[1] == 0.:
                                    print('cur_gtvel: {}, cur_genvel: {}'.format(cur_gtvel, cur_genvel))
                            input('enter to continue')
                        else:
                            _, gen_cost, layer_cost, gen_l1_cost, summary, gen_l2_cost = result_list
                        # if False:
                        #     print('pposx: {}, pposy: {}, pvels: {}'.format(pposx_res.shape, pposy_res.shape, pvels_res.shape))
                        #     print('genXs shape: {}, gtXs shape: {}'.format(gen_Xs.shape, gt_Xs.shape))
                        #     print('ppos shape: {}'.format(ppos_res.shape))
                        #     print('bl_id_res: {}, Ibl_res: {}, cxl_res: {}, wa_res: {}'.format(bl_id_res.shape, Ibl_res.shape, cxl_res.shape, wa_res.shape))
                        #     input('')
                    else:
            ...
            if (iteration + 1) % valiInterval == 0:
                if use_spatialdisc:
                    ...
                    # get vali data
                    cur_ppos, batch_xs, batch_ys = getInput(batch_size = numValis, isTraining=False)
