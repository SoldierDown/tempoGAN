...
def weight_variable(shape, init_weights=None):
	...
	initial = tf.random.truncated_normal(**params)
...
def variable_summaries(var, name):
	...
	with tf.compat.v1.name_scope('summaries'):
		...
		tf.compat.v1.summary.scalar(name='mean/' + name, tensor=mean)
		with tf.compat.v1.name_scope('stddev'):
			stddev = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(var - mean)))

		tf.compat.v1.summary.scalar(name='stddev/' + name, tensor=stddev)
		tf.compat.v1.summary.scalar(name='max/' + name, tensor=tf.reduce_max(input_tensor=var))
		tf.compat.v1.summary.scalar(name='min/' + name, tensor=tf.reduce_min(input_tensor=var))
		tf.compat.v1.summary.histogram(name=name, values=var)
...
def nn_layer(input_tensor, input_dim, output_dim, layer_name,
	...
	with tf.compat.v1.name_scope(layer_name):
		...
		with tf.compat.v1.name_scope('Wx_plus_b' if not bn else 'BN_Wx_plus_b'):
			if bn:
				...
				if is_training:
					decay         = 0.999
					b_mean, b_var = tf.nn.moments(x=z_bn, axes=[0])
					t_mean        = tf.compat.v1.assign(p_mean, p_mean*decay + b_mean*(1.0-decay))
					t_var         = tf.compat.v1.assign(p_var, p_var*decay + b_var*(1.0-decay))
			...
			tf.compat.v1.summary.histogram(name=layer_name + '/pre_activations', values=preactivate)

		activations = act(preactivate, 'activation')
		tf.compat.v1.summary.histogram(name=layer_name + '/activations', values=activations)
		return activations
...
def build_network(layers, layers_act=None, input_x_holder=None, input_y_holder=None, dropout_holder=None,
	...
	with tf.compat.v1.name_scope('{}input'.format(scope)):
		x  = tf.compat.v1.placeholder(tf.float32, shape=[None, layers[ 0]], name='x-input') if input_x_holder is None else input_x_holder
		y_ = tf.compat.v1.placeholder(tf.float32, shape=[None, layers[-1]], name='y-input') if input_y_holder is None else input_y_holder
