...
x          = tf.compat.v1.placeholder(tf.float32, shape=[None, dlayers[0]], name='x-input')
keep_prob  = tf.compat.v1.placeholder(tf.float32, name='keep_prob_detector') if pargs.ddrop>0.0 else None
keep_prob2 = tf.compat.v1.placeholder(tf.float32, name='keep_prob_modifier') if pargs.mdrop>0.0 else None
...
with tf.compat.v1.name_scope('accuracy'):
	with tf.compat.v1.name_scope('correct_prediction'):
		...
		else:            corr, appx = tf.argmax(input=y_, axis=1), tf.argmax(input=y, axis=1)                                       # 0: splashing, 1: non-splashing
		...
		N_corr_non_splas    = tf.reduce_sum(input_tensor=corr)
		N_corr_splashing    = tf.cast(tf.shape(input=y_)[0], tf.int64) - N_corr_non_splas
		diff_appx_corr      = tf.math.logical_xor(y_b_appx, y_b_corr)
		N_corr_spl_appx_non = tf.reduce_sum(input_tensor=tf.cast(tf.logical_and(y_b_corr, diff_appx_corr), tf.float32))
		N_corr_non_appx_spl = tf.reduce_sum(input_tensor=tf.cast(tf.logical_and(y_b_appx, diff_appx_corr), tf.float32))
		...
		log_dict['splashes/corr']                = 1.0 - tf.reduce_mean(input_tensor=tf.cast(corr, tf.float32))
		log_dict['splashes/appx']                = 1.0 - tf.reduce_mean(input_tensor=tf.cast(appx, tf.float32))

	with tf.compat.v1.name_scope('loss'):
		loss_normalizer = 1.0/tf.cast(tf.shape(input=y2_)[0], tf.float32)

		with tf.compat.v1.name_scope('detector'):
			if pargs.nosmax: loss_detector = tf.nn.l2_loss(y - y_)
			else:            loss_detector = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=y, labels=tf.stop_gradient(y_)))
			log_dict['detector/loss'] = loss_detector*loss_normalizer if pargs.nosmax else loss_detector

		with tf.compat.v1.name_scope('modifier'):
			loss_modifier = tf.nn.l2_loss(y2 - y2_)
			log_dict['modifier/loss'] = loss_modifier*loss_normalizer
			if pargs.mve:
				loss_modifier_mve = 0.5*tf.reduce_sum(input_tensor=((y2 - y2_)**2)/(s**2 + 1e-4)) + 0.5*tf.reduce_sum(input_tensor=tf.math.log(s**2 + 1e-4)) # mean variance estimate
	...
	for i in log_dict:
		tf.compat.v1.summary.scalar(name=i, tensor=log_dict[i])

with tf.compat.v1.name_scope('train'):
	train_step = tf.compat.v1.train.AdamOptimizer(1e-4).minimize(loss)
	if pargs.mve: train_step_mve = tf.compat.v1.train.AdamOptimizer(1e-4).minimize(loss_mve)

merged = tf.compat.v1.summary.merge_all()
train_writer = tf.compat.v1.summary.FileWriter(pargs.output + '/summary/train', sess.graph)
test_writer  = tf.compat.v1.summary.FileWriter(pargs.output + '/summary/test')
...
for i in range(pargs.steps):
	if (i%10==0):           # test
		...
		if (i%100==99):
			...
			params_sess_run.update(options=tf.compat.v1.RunOptions(trace_level=tf.compat.v1.RunOptions.FULL_TRACE), run_metadata=run_metadata)
