...
#input for gen
# x = tf.Variable(tf.ones(shape=[None, n_input]), dtype=tf.float32)
x = tf.Variable(tf.ones(shape=[n_input]), dtype=tf.float32)
#reference input for disc
# x_disc = tf.Variable(tf.ones(shape=[None, n_input]), dtype=tf.float32)
x_disc = tf.Variable(tf.ones(shape=[n_input]), dtype=tf.float32)
#real input for disc
# y = tf.Variable(tf.ones(shape=[None, n_output]), dtype=tf.float32)
y = tf.Variable(tf.ones(shape=[n_output]), dtype=tf.float32)
"""
kk = tf.Variable(dtype=tf.float32)
kk2 = tf.Variable(dtype=tf.float32)
kkt = tf.Variable(dtype=tf.float32)
kktl = tf.Variable(dtype=tf.float32)
"""
kk = tf.Variable(0., dtype=tf.float32)
kk2 = tf.Variable(0., dtype=tf.float32)
kkt = tf.Variable(0., dtype=tf.float32)
kktl = tf.Variable(0., dtype=tf.float32)
...
# train = tf.Variable(dtype=tf.bool)
train = tf.Variable(False, dtype=tf.bool)
...
if not outputOnly:
	...
	if decayLR:
		learning_rate = tf.compat.v1.train.polynomial_decay(learning_rate, lr_global_step, trainingIters//2, learning_rate_scalar*0.05, power=1.1)
	...
	if (useTempoD or useTempoL2):# temporal loss here
		...
		with tf.device(device_str): 
			x_t = tf.Variable(tf.ones(shape=[None, n_input]), dtype=tf.float32)
			gen_part_t = gen_model(x_t, reuse=True, use_batch_norm=bn, train=train)
		if(ADV_flag):
			y_pos = tf.Variable(tf.ones(shape=[None, n_output * dataDimension]), dtype=tf.float32)
			if dataDimension == 2:
		...
		if (useTempoD):
			# real input for disc
			y_t = tf.Variable(tf.ones(shape=[None, n_output]), dtype=tf.float32)
			if (ADV_flag):
				if dataDimension == 2:
...
if not outputOnly:
	...
	#testing losses
	if use_spatialdisc:
		lossVali_disc_disc   = tf.compat.v1.summary.scalar("discriminator-loss vali real", disc_loss_disc)
		lossVali_disc_gen   = tf.compat.v1.summary.scalar("discriminator-loss vali generated", disc_loss_gen)
		lossVali_disc = tf.compat.v1.summary.scalar("discriminator-loss vali", disc_loss)
		lossVali_gen   = tf.compat.v1.summary.scalar("generator-loss vali", gen_loss)
	...
	if use_spatialdisc:
		outVali_disc_real = tf.compat.v1.summary.scalar("discriminator-out vali", disc_sigmoid)
		outVali_disc_gen = tf.compat.v1.summary.scalar("generator-out vali", gen_sigmoid)
	...
	if(useTempoD): # all temporal losses
		...
		lossVali_disc_disc_t = tf.compat.v1.summary.scalar("T discriminator-loss vali real", t_disc_loss_disc)
		lossVali_disc_gen_t = tf.compat.v1.summary.scalar("T discriminator-loss vali generated", t_disc_loss_gen)
		lossVali_disc_t = tf.compat.v1.summary.scalar("T discriminator-loss vali", t_disc_loss)
		lossVali_gen_t = tf.compat.v1.summary.scalar("T generator-loss vali", t_gen_loss)
		...
		outVali_disc_real_t = tf.compat.v1.summary.scalar("T discriminator-out vali", t_disc_sigmoid)
		# discriminator output [0,1] for generated input
		outVali_disc_gen_t = tf.compat.v1.summary.scalar("T generator-out vali", t_gen_sigmoid)
	
	if (useTempoL2):  # all temporal losses
		lossTrain_gen_t_l = tf.compat.v1.summary.scalar("T generator-loss train l2", tl_gen_loss)
		lossVali_gen_t_l = tf.compat.v1.summary.scalar("T generator-loss vali l2", tl_gen_loss)
