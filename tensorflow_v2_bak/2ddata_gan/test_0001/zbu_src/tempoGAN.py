...
#input for gen
x = tf.Variable(tf.float32, tf.ones(shape=[None, n_input]))
#reference input for disc
x_disc = tf.Variable(tf.float32, tf.ones(shape=[None, n_input]))
#real input for disc
y = tf.Variable(tf.float32, tf.ones(shape=[None, n_output]))
kk = tf.Variable(tf.float32)
kk2 = tf.Variable(tf.float32)
kkt = tf.Variable(tf.float32)
kktl = tf.Variable(tf.float32)
...
if not outputOnly:
	...
	if (useTempoD or useTempoL2):# temporal loss here
		...
		with tf.device(device_str): 
			x_t = tf.Variable(tf.float32, shape=[None, n_input])
			gen_part_t = gen_model(x_t, reuse=True, use_batch_norm=bn, train=train)
		if(ADV_flag):
			y_pos = tf.Variable(tf.float32, tf.ones(shape=[None, n_output * dataDimension]))
			if dataDimension == 2:
		...
		if (useTempoD):
			# real input for disc
			y_t = tf.Variable(tf.float32, tf.ones(shape=[None, n_output]))
			if (ADV_flag):
				if dataDimension == 2:
