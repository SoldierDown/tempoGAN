...
#input for gen
x = tf.Variable(tf.float32, shape=[None, n_input])
#reference input for disc
x_disc = tf.Variable(tf.float32, shape=[None, n_input])
#real input for disc
y = tf.Variable(tf.float32, shape=[None, n_output])
...
def disc_binclass(in_low, in_high, reuse=False, use_batch_norm=False, train=None):
	...
	#train: if use_batch_norm, tf bool Variable
...
if not outputOnly:
	...
	if (useTempoD or useTempoL2):# temporal loss here
		...
		if(ADV_flag):
			y_pos = tf.Variable(tf.float32, shape=[None, n_output * dataDimension])
			if dataDimension == 2:
		...
		if (useTempoD):
			# real input for disc
			y_t = tf.Variable(tf.float32, shape=[None, n_output])
			if (ADV_flag):
				if dataDimension == 2:
