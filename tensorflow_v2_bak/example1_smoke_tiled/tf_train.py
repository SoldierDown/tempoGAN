...
# ---------------------------------------------
# TENSORFLOW SETUP
x      = tf.compat.v1.placeholder(tf.float32, shape=[None, 1, tileSizeLow, tileSizeLow, n_inputChannels])  # fixed to 2D for now
y_true = tf.compat.v1.placeholder(tf.float32, shape=[None, 1, tileSizeHigh,tileSizeHigh, 1             ])  # also fixed, always output 1 channel
training = tf.compat.v1.placeholder(tf.bool)

hl1_size = 512 
xIn = tf.reshape(x, shape=[-1, n_input ]) # flatten
fc_1_weight = tf.Variable(tf.random.normal([n_input, hl1_size], stddev=0.01))
fc_1_bias   = tf.Variable(tf.random.normal([hl1_size], stddev=0.01))

fc1 = tf.add(tf.matmul(xIn, fc_1_weight), fc_1_bias)
fc1 = tf.nn.tanh(fc1)
fc1 = tf.compat.v1.layers.dropout(fc1, rate=dropout, training=training)

fc_out_weight = tf.Variable(tf.random.normal([hl1_size, tileSizeHigh * tileSizeHigh], stddev=0.01))
fc_out_bias   = tf.Variable(tf.random.normal([tileSizeHigh * tileSizeHigh], stddev=0.01))
...
optimizer = tf.compat.v1.train.AdamOptimizer(learningRate).minimize(costFunc)
...
lossTrain  = tf.compat.v1.summary.scalar("loss",     costFunc)
lossTest   = tf.compat.v1.summary.scalar("lossTest", costFunc)
