...
y = tf.compat.v1.placeholder(tf.float32, shape=[None, 64,64, 1])

xIn = tf.reshape(x, shape=[-1, inSize ]) # flatten
fc_1w = tf.Variable(tf.random.normal([inSize, 50], stddev=0.01))
fc_1b   = tf.Variable(tf.random.normal([50], stddev=0.01))

fc1 = tf.add(tf.matmul(xIn, fc_1w), fc_1b)
fc1 = tf.nn.tanh(fc1)
fc1 = tf.nn.dropout(fc1, rate=1 - (0.5)) # plenty of dropout...

fc_2w = tf.Variable(tf.random.normal([50, inSize], stddev=0.01))  # back to input size
fc_2b = tf.Variable(tf.random.normal([inSize], stddev=0.01))

y_pred = tf.add(tf.matmul(fc1, fc_2w), fc_2b)
y_pred = tf.reshape( y_pred, shape=[-1, 64, 64, 1])

cost = tf.nn.l2_loss(y - y_pred) 
opt  = tf.compat.v1.train.AdamOptimizer(0.0001).minimize(cost)
