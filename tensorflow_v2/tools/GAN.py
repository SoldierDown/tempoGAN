...
class GAN(object):
	...
	def convolutional_layer(self, outChannels, _patchShape, activation_function=tf.nn.tanh, stride=[1], name="conv",reuse=False, batch_norm=False, train=None, in_layer=None):
		...
		with tf.compat.v1.variable_scope(name):
			...
			if batch_norm:
				...
				self.layer = tf.compat.v1.contrib.layers.batch_norm(self.layer, decay=self.bn_decay, scale=True, scope=tf.compat.v1.get_variable_scope(), reuse=reuse, fused=False, is_training=train)
				# self.layer = tf.layers.batch_norm(self.layer, decay=self.bn_decay, scale=True, scope=tf.compat.v1.get_variable_scope(), reuse=reuse, fused=False, is_training=train)
	...
	def avg_depool(self, window_size=[2, 2], window_stride=[2,2]):
		...
		self.layer = tf.image.resize(self.layer, [int(outHeight), int(outWidth)], 0) #0 = ResizeMethod.BILINEAR
		...
		if is3D: # recover D dimension
			...
			self.layer = tf.image.resize(self.layer, [int(s[1]*window_size[0]), int(s[2])], 0) #0 = ResizeMethod.BILINEAR		
	...
	def noise(self, channels=-1):
		...
		if channels > 0:
			shape[-1] = channels
		noise = tf.random.normal(shape=shape, mean=0.0, stddev=0.04, dtype=tf.float32)
	...
	def weight_variable(self, shape, name="w", init_mean=0.):
		...
		if init_mean==1.:
			s = 0. # enforce value
		v = tf.compat.v1.get_variable("weight", shape, initializer=tf.compat.v1.random_normal_initializer(stddev=s, mean=init_mean))
		return v

	#---------------------------------------------------------------------------------
	# gemerate biases for the nodes
	def bias_variable(self, shape, name="b"):
		return tf.compat.v1.get_variable("bias", shape, initializer=tf.compat.v1.constant_initializer(0.1))
